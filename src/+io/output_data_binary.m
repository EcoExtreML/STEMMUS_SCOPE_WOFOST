function n_col = output_data_binary(f, k, xyt, rad,  canopy, V, vi, vmax, options, fluxes, meteo, iter, thermal,spectral, gap, profiles, Sim_Theta_U, Sim_Temp, Trap, Evap,crop_output,sfactor)
%% OUTPUT DATA
...
flu_out = [k iter.counter xyt.year(k) xyt.t(k) fluxes.Rntot fluxes.lEtot fluxes.Htot fluxes.Rnctot fluxes.lEctot, ...
    ...
    1E6*fluxes.aPAR_Cab fluxes.aPAR/rad.PAR fluxes.aPAR_Wm2 1E6*rad.PAR rad.Eoutf rad.Eoutf./fluxes.aPAR_Wm2 Trap(k)*10 Evap(k)*10 Trap(k)*10+Evap(k)*10 fluxes.GPP fluxes.NEE sfactor];
...
%% aerodyn
aerodyn_out =  [k xyt.year(k) xyt.t(k) thermal.raa, thermal.rawc, thermal.raws, thermal.ustar, thermal.raa+thermal.rawc, thermal.raa+thermal.raws, thermal.rscu, thermal.rsch, thermal.rss];
