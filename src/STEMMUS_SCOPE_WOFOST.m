...
for i = 1:1:Dur_tot
    ...
    % using simulated LAI and PH (LAI>=LAIEM or PH>=PHEM to keep the stability)
   ...
   [crop_output] = wofost.wofostdebug(wofostpar,V,xyt,options);
    ...
    kk=k;
    n_col = io.output_data_binary(f, k, xyt, rad, canopy, V, vi, vmax, options, fluxes, meteo, iter, thermal, spectral, gap, profiles, Sim_Theta_U, Sim_Temp, Trap, Evap,crop_output,sfactor);
end
