...
% Filter the Start time if the initial LAI is too large
initial_lai = lai_data(indices_peak_left(1));
initial_threshold = lai_data(filtered_valley_indices(2))+(lai_data(filtered_peak_indices(1))-lai_data(filtered_valley_indices(2)))*1/3;

if initial_lai > initial_threshold
    indices_peak_left = indices_peak_left(2:end);
    indices_peak_right = indices_peak_right(2:end);
    filtered_peaks = filtered_peaks(2:end);
    filtered_peak_indices = filtered_peak_indices(2:end);
end
...
% calculate the TSUMSTART
TSUMSTART = [];
unique_years = unique(years);
if initial_lai > initial_threshold       % discard the data of the first year
    unique_years = unique_years(2:end);
end

for n = 1:numel(unique_years)
    year = unique_years(n);
    year_indices = find(years == year);
    TSUMSTART(n) = sum(TaEffective(year_indices(1):CSTART(n)))/NstepDay;
end

% update the crop parameters for time series
wofostpar.CSTARTSeries = CSTART;
wofostpar.CENDSeries   = CEND;
wofostpar.TSUMSTARTSeries = TSUMSTART;
