...
Actot           = LAI*(Fc*Ah   + equations.meanleaf(canopy,Au  ,'angles_and_layers',Ps)); % photosynthesis leaves Ag-Rd
Atot            = LAI*(Fc*Ahh   + equations.meanleaf(canopy,Auu  ,'angles_and_layers',Ps)); % photosynthesis leaves Ag
...
fluxes.GPP      = Atot*12/1000000000;  % [kg C m-2 s-1]      gross primary production
...
thermal.rss   = rss;      % [s m-1]             stomatal resistance of soil 
thermal.rscu  = mean(rcwu,'All');   % [s m-1]   stomatal resistance of sunlit leaves
thermal.rsch  = mean(rcwh,'All');   % [s m-1]   stomatal resistance of shaded leaves 
thermal.Tcu   = Tcu;
thermal.Tch   = Tch;
